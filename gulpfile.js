...
gulp.task('styles', () => {
	return gulp
		.src('./assets/css/components.scss', {allowEmpty: true})
		...
		//.pipe(gulp.dest(config.styleDestinationAlt))
...
gulp.task('vendorsJS', () => {
	return gulp
		...
		//.pipe(gulp.dest(config.jsVendorDestinationAlt))
		.pipe(
			rename({
...
gulp.task('parallaxJS', () => {
	return gulp
		.src(config.jsParallaxSRC, {since: gulp.lastRun('parallaxJS')}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(remember(config.jsParallaxSRC)) // Bring all files back to stream.
		...
		.pipe(
			rename({
				basename: config.jsParallaxFile,
				suffix: '.min'
			})
		)
		.pipe(terser())
...
gulp.task('customJS', () => {
	return gulp
		.src(config.jsCustomSRC, {since: gulp.lastRun('customJS'), sourcemaps: true}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(remember(config.jsCustomSRC)) // Bring all files back to stream.
		.pipe(concat(config.jsCustomFile + '.js'))
		.pipe(lineec()) // Consistent Line Endings for non UNIX systems.
		.pipe(gulp.dest(config.jsCustomDestination, {sourcemaps: true}))
		//.pipe(gulp.dest(config.jsCustomDestinationAlt, {sourcemaps: true}))
		.pipe(
			rename({
		...
		//.pipe(gulp.dest(config.jsCustomDestinationAlt))
		.pipe(
			notify({
				"message": "\n\n✅  ===> CUSTOM JS — completed!\n",
				"onLast": true
...
/* 		.pipe(
			cache(
				imagemin([
					...
					imagemin.optipng({optimizationLevel: 6}), // 0-7 low-high.
					imagemin.svgo({
		...
		//.pipe(gulp.dest(config.imgDSTAlt))
		.pipe(
			notify({
				"message": "\n\n✅  ===> IMAGES — completed!\n",
				"onLast": true
...
gulp.task(
	'default',
	gulp.parallel('html', 'styles', 'parallaxJS', 'vendorsJS', 'customJS', 'images', browsersync, () => {
		gulp.watch(config.watchPhp, reload); // Reload on PHP file changes.
		//gulp.watch(config.watchHtml, gulp.parallel('html')); // Reload on SCSS file changes.
		gulp.watch('html/src/**/*', gulp.series('html', reload));
		gulp.watch(config.watchStyles, gulp.parallel('styles')); // Reload on SCSS file changes.
		gulp.watch(config.watchJsParallax, gulp.series('parallaxJS', reload)); // Reload on vendorsJS file changes.
...
// Build task - Generates build compiled assets
gulp.task('build', gulp.series('html', 'styles', 'parallaxJS', 'vendorsJS', 'customJS', 'images'));